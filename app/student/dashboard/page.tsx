            ...
            {activeSection === "attendance" && (
                <Card className="border-0 bg-slate-800/50 backdrop-blur-xl shadow-xl">
                    ...
                    <CardContent>
                        ...
                        <div className="space-y-2">{attendance.slice(0, 10).map((r: any) => (<div key={r.id} className="flex justify-between items-center p-3 border border-slate-700 rounded-lg"><div><div className="text-sm font-medium text-white">{r.subject_name || r.class_name}</div><div className="text-[10px] text-slate-500">{new Date(r.attendance_date).toLocaleDateString()}</div></div><Badge className={getAttendanceStatusColor(r.status)}>{r.status}</Badge></div>))}</div>
                    </CardContent>
                </Card>
            )}
